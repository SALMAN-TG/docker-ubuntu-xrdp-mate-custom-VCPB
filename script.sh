#!/bin/bash
if [ $UID -eq 0 ]; then
user=$1salman #Make change here if you changed username
exec su "$user" "$0" -- "$@"
fi
echo "This will be run from user $UID"
cp -f /home/__init__.py /home/vcbot/config/__init__.py
mv /home/Telegram /home/salman/Telegram #Move Telegram to salman's Home (make changes according to username)
